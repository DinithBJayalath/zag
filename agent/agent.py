...
def generate_command(query):
    ...
    response = LLM_client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You're a terminal assistant. Translate the following user query into a shell command. Reply with only a bash command inside triple backticks (```bash), followed by a clear explanation of what the command does."},
